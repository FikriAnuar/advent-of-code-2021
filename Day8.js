...
for (var i = 0; i < patterns.length; i++) {
  if (i % 15 > 10 && i % 15 < 15) {
    if (patterns[i].length === 2 || patterns[i].length === 3 ||patterns[i].length === 4 || patterns[i].length === 7) {
      total += 1;
    }
  }
}
...
var sum = 0;
for (var i = 0; i < patterns.length; i+=15) {
  var numbers = {};
  var letters = {};
  var ones = {}
  var bd = [];
  ...
  for (var j = i; j < i + 15; j++) {
    if (patterns[j].length === 2) {
      numbers[patterns[j].split('').sort().join('')] = 1;
      for (var k = 0; k < patterns[j].length; k++) {
        ones[patterns[j][k]] = true;
      }
    } else if (patterns[j].length === 3) {
      numbers[patterns[j].split('').sort().join('')] = 7;
      sevenString = patterns[j].split('').sort().join('');
    } else if (patterns[j].length === 4) {
      numbers[patterns[j].split('').sort().join('')] = 4;
      fourString = patterns[j].split('').sort().join('');
    } else if (patterns[j].length === 7) {
      numbers[patterns[j].split('').sort().join('')] = 8;
    }
  }
  for (var k = 0; k < sevenString.length; k++) {
    if (!ones[sevenString[k]]) {
      letters[sevenString[k]] = 'a';
    }
  }
  for (var k = 0; k < fourString.length; k++) {
    if (!ones[fourString[k]]) {
      bd.push(fourString[k]);
    }
  }
  for (var j = i; j < i + 15; j++) {
    if (patterns[j].length === 5) {
      var difference = 0;
      var secDiff = 0;
      for (var k = 0; k < patterns[j].length; k++) {
        if (!ones[patterns[j][k]]) {
          difference++;
        }
        if (!bd.includes(patterns[j][k])) {
          secDiff++;
        }
      }
      if (difference === 3) {
        numbers[patterns[j].split('').sort().join('')] = 3;
      } else {
        if (secDiff === 4) {
          numbers[patterns[j].split('').sort().join('')] = 2;
        } else {
          numbers[patterns[j].split('').sort().join('')] = 5;
        }
      }
    }
  }
  for (var j = i; j < i + 15; j++) {
    if (patterns[j].length === 6) {
      ...
      if (difference === 5) {
        numbers[patterns[j].split('').sort().join('')] = 6;
      } else {
        if (secDiff === 4) {
          numbers[patterns[j].split('').sort().join('')] = 9;
        } else {
          numbers[patterns[j].split('').sort().join('')] = 0;
        }
      }
    }
  }
  var string = '';
  for (var j = i + 11; j < i + 15; j++) {
    string += numbers[patterns[j].split('').sort().join('')];
  }
  console.log(string);
  sum += parseInt(string);
}
